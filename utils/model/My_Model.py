...
def fcm(A):
    ...
    # 对 AAT 进行 softmax 操作
    softmax_AAT = F.softmax(AAT, dim=1)

    # 将 softmax_AAT 乘到原始的 A 上
    A_softmax = torch.matmul(softmax_AAT, A)

    # 将 A_softmax 加到原始的 A 上
    result = A + A_softmax
    return result
...
def colorful_spectrum_mix(img1, img2, alpha, ratio=1.0):
    ...
    # lam_peo = 1-lam_noise
    # lam_noise = torch.ones((img1.shape[0], img1.shape[1], 1, 1)).fill_(1 - alpha).cuda()
    # lam_peo = torch.ones((img1.shape[0], img1.shape[1], 1, 1)).fill_(alpha).cuda()
...
def KL_loss(generate_vic,target_vic):
    ...
    distribution1 = nn.functional.softmax(generate_vic, dim=1)
    distribution2 = nn.functional.softmax(target_vic, dim=1)
    loss = kld_loss(torch.log(distribution1), distribution2)
    return loss
...
class DG_Net(nn.Module):
    def __init__(self,model1,model2,num_classes):#####  ,GGLM1,GGLM2
        super(DG_Net, self).__init__()
        self.E1 = model1
        self.E2 = model2
        self.KL_defen1 = KL_defen(model1.layer1)
        self.KL_defen2 = KL_defen(model2.layer1)

        self.num_classes = num_classes
        self.in_planes = 384

        # self.GGLM = GGLM(self.model1.layer2, self.model2.layer2, self.num_classes)
        ...
        self.classifier_final2 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_final2.apply(weights_init_classifier)

        self.classifier_final3 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_final3.apply(weights_init_classifier)

        self.classifier_final4 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_final4.apply(weights_init_classifier)

        self.classifier_final5 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_final5.apply(weights_init_classifier)

        self.classifier_final6 = nn.Linear(self.in_planes, self.num_classes, bias=False)
        self.classifier_final6.apply(weights_init_classifier)
        ...
        self.BN_fin2 = nn.BatchNorm1d(self.in_planes)
        self.BN_fin2.bias.requires_grad_(False)
        self.BN_fin2.apply(weights_init_kaiming)

        self.BN_fin3 = nn.BatchNorm1d(self.in_planes)
        self.BN_fin3.bias.requires_grad_(False)
        self.BN_fin3.apply(weights_init_kaiming)

        self.BN_fin4 = nn.BatchNorm1d(self.in_planes)
        self.BN_fin4.bias.requires_grad_(False)
        self.BN_fin4.apply(weights_init_kaiming)
        self.layer_norm = copy.deepcopy(model1.norm)
    ...
    def forward(self, x, x_noise):
        ...
        global_x_1 = self.E1(x)
        global_x_1_noise = self.E1(x_noise)

        global_x_2 = self.E2(x)
        global_x_2_noise = self.E2(x_noise)

        #### 防御的Token

        if self.training:

            kl_loss1 = self.KL_defen1(global_x_1,global_x_1_noise)
            kl_loss2 = self.KL_defen2(global_x_2,global_x_2_noise)

            x_1 = global_x_1[:, 0]
            x_1_noise = global_x_1_noise[:, 0]

            x_2 = global_x_2[:, 0]
            x_2_noise = global_x_2_noise[:, 0]
            ...
            score_fin2 = self.classifier_final2(self.BN_fin2(x_2))
            score_fin2_noise = self.classifier_final2(self.BN_fin2(x_2_noise))

            score_noise1 = self.classifier_final3((self.BN_fin3(x_1_noise)))
            score_noise2 = self.classifier_final3((self.BN_fin3(x_2_noise)))

            x_3_noise = (x_1_noise + x_2_noise) / 2


            score_noise3 = self.classifier_final4((self.BN_fin4(x_3_noise)))


            feat_list = [x_1, x_1_noise,x_2,x_2_noise,x_3_noise]
            score_list = [score_fin1, score_fin1_noise,score_fin2,score_fin2_noise,score_noise1,score_noise2,score_noise3]

            kl_loss = (kl_loss1 + kl_loss2)/2.0

            return feat_list, score_list,kl_loss
        else:
            result = global_x_2_noise[:, 0]
            return result
